...
with sync_playwright() as pw:
    ...
    for width in (320,390,1440):
        ...
        page=context.new_page(); errors=[]; submitted=[]; fail_register=[width==320]
            ...
            if path.startswith('/api/fleet/'):
                ...
                if action=='register':
                    if fail_register[0]:
                        ...
                        return route.fulfill(status=502,json={'detail':'注册暂时失败，请重试'})
        ...
        assert page.locator('#fleet-register').is_enabled()
        assert page.locator('#fleet-register').inner_text()=='保存并注册应用'
        ...
        assert page.locator('#fleet-password').evaluate('(el)=>el===document.activeElement')
        ...
        if width==320:
            page.wait_for_function("document.querySelector('#fleet-message').textContent.includes('注册暂时失败')")
            assert page.locator('#fleet-register').is_enabled()
            assert page.locator('#fleet-register').inner_text()=='注册应用'
            page.locator('#fleet-register').click()
        ...
        assert submitted==(['config','register','register','paired'] if width==320 else ['config','register','paired'])
        if width==390:
